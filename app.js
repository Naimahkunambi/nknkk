...
function updateUI() {
  ...
  if (!state.touch || !state.noTouch) {
    $('combinedCost').textContent = '—';
    $('lockedEdge').textContent = '—';
    $('stabilityScore').textContent = String(state.stableCount);
    return;
  }

  const combined = state.touch.ask + state.noTouch.ask;
  const winningPayoutFloor = Math.min(state.touch.payout, state.noTouch.payout);
  const edge = winningPayoutFloor - combined;
...
function evaluatePair() {
  ...
  if (qualified) {
    if (signature !== state.lastQualifiedSignature) {
      state.stableCount += 1;
      state.lastQualifiedSignature = signature;
    }
  } else {
    state.stableCount = 0;
    state.lastQualifiedSignature = '';
  }

  $('stabilityScore').textContent = String(state.stableCount);

  if (!fresh) {
    setStatus('Waiting for fresh quotes', 'Both legs must be fresh before the engine can arm.', 'neutral');
    return;
  }

  if (!qualified) {
    setStatus('No arbitrage', `Current pair edge ${edge.toFixed(4)} USD is below your ${s.minEdge.toFixed(4)} USD minimum.`, 'neutral');
    return;
  }

  if (state.stableCount < s.stableChecks) {
    setStatus('Candidate detected', `Positive edge ${edge.toFixed(4)} USD. Confirming stability ${state.stableCount}/${s.stableChecks}.`, 'armed');
    return;
  }
  ...
  if (state.connectedDemo && $('autoExecute').checked && !state.executing) {
    executePair().catch(err => {
      log(`Execution error: ${err.message}`, 'bad');
      emergencyStop(`Execution error: ${err.message}`);
    });
  }
}
...
function handleMessage(event) {
  ...
  if (msg.error) {
    ...
    if (pending) {
      ...
      pending.reject(new Error(msg.error.message || msg.error.code || 'Deriv API error'));
    } else {
      log(`Deriv error: ${msg.error.message || msg.error.code}`, 'bad');
    }
    return;
  }
...
function subscribePair() {
  resetQuotes();
  send(proposalRequest('ONETOUCH', 101));
  send(proposalRequest('NOTOUCH', 102));
  state.scanning = true;
...
async function startPublicScanner() {
  ...
  $('autoExecute').checked = false;
  $('autoExecute').disabled = true;
  await openSocket(PUBLIC_WS, 'public');
  subscribePair();
}
...
async function getDemoSocketUrl() {
  ...
  if (!appId || !accountId || !token) throw new Error('App ID, demo account ID and access token are required.');
  ...
  if (!response.ok) {
    const message = body?.errors?.[0]?.message || `OTP request failed (${response.status}).`;
    throw new Error(message);
  }
  const url = body?.data?.url;
  if (!url) throw new Error('Deriv did not return a WebSocket URL.');
  if (!url.includes('/ws/demo')) throw new Error('Safety lock: this app only accepts Deriv DEMO WebSocket URLs.');
  return url;
}

async function connectDemo() {
  state.killed = false;
  setStatus('Connecting demo', 'Requesting one-time authenticated WebSocket URL from Deriv.', 'neutral');
  const url = await getDemoSocketUrl();
  await openSocket(url, 'demo');
  state.connectedDemo = true;
  $('connectDemoBtn').disabled = true;
  $('disconnectDemoBtn').disabled = false;
  $('autoExecute').disabled = false;
  $('emergencyBtn').disabled = false;
  subscribePair();
  setStatus('Demo connected', 'Live proposals are authenticated. Auto-execution remains off until you enable it.', 'live');
  log('Connected to Deriv DEMO. Real-money WebSocket URLs are blocked by the app.', 'good');
}

function extractContractId(msg) {
  const id = Number(msg?.buy?.contract_id);
  return Number.isFinite(id) ? id : null;
}

async function buyProposal(proposal, label) {
  const msg = await request({ buy: proposal.id, price: proposal.ask }, 10000);
  const contractId = extractContractId(msg);
  if (!contractId) throw new Error(`${label} buy response did not contain a contract ID.`);
  log(`${label} filled. Contract ${contractId}, buy price ${money(Number(msg.buy?.buy_price))}.`, 'good');
  return { label, contractId, buy: msg.buy };
}

async function sellAtMarket(contractId, label) {
  try {
    const msg = await request({ sell: contractId, price: 0 }, 7000);
    log(`Emergency sell sent for ${label} contract ${contractId}.`, 'warn');
    return msg;
  } catch (err) {
    log(`Emergency sell FAILED for ${label} contract ${contractId}: ${err.message}`, 'bad');
    throw err;
  }
}
...
async function monitorContract(contractId, label) {
  try {
    send({ proposal_open_contract: 1, contract_id: contractId, subscribe: 1, req_id: ++state.reqSeq, passthrough: { label } });
  } catch (err) {
    log(`Could not start monitor for ${label}: ${err.message}`, 'warn');
  }
}
...
async function executePair() {
  ...
  const noTouch = { ...state.noTouch };
  if (!touch || !noTouch) throw new Error('Both proposals are required.');

  const combined = touch.ask + noTouch.ask;
  const payoutFloor = Math.min(touch.payout, noTouch.payout);
  const edge = payoutFloor - combined;
  ...
  const results = await Promise.allSettled([
    buyProposal(touch, 'TOUCH'),
    buyProposal(noTouch, 'NO TOUCH'),
  ]);
  ...
  if (fulfilled.length === 2) {
    state.currentPair = fulfilled;
    fulfilled.forEach(x => monitorContract(x.contractId, x.label));
    log('PAIR CONFIRMED: both legs filled. Hedge is now established for this pair.', 'good');
  ...
  if (fulfilled.length === 1) {
    const lone = fulfilled[0];
    log(`CRITICAL: only ${lone.label} filled. Attempting emergency sell.`, 'bad');
    try { await sellAtMarket(lone.contractId, lone.label); } catch {}
  } else {
    log('Both buy requests failed. No confirmed open pair.', 'bad');
  }

  failed.forEach(r => log(`Failed leg: ${r.reason?.message || r.reason}`, 'bad'));
  state.executing = false;
}
...
function emergencyStop(reason = 'Emergency stop pressed by user.') {
  ...
  setStatus('ENGINE STOPPED', reason, 'risk');
  log(reason, 'bad');
}
...
function stopScanner() {
  ...
  $('startScannerBtn').disabled = false;
  $('stopScannerBtn').disabled = true;
  setStatus('Scanner stopped', 'No execution will occur while scanner is stopped.', 'neutral');
  log('Scanner stopped.');
}

function disconnectDemo() {
  emergencyStop('Demo disconnected.');
  closeSocket();
  state.connectedDemo = false;
  $('connectDemoBtn').disabled = false;
  $('disconnectDemoBtn').disabled = true;
...
$('startScannerBtn').addEventListener('click', async () => {
  try { await startPublicScanner(); } catch (err) { log(err.message, 'bad'); setStatus('Scanner error', err.message, 'risk'); }
});
$('stopScannerBtn').addEventListener('click', stopScanner);
$('connectDemoBtn').addEventListener('click', async () => {
  try { await connectDemo(); } catch (err) { log(`Demo connection failed: ${err.message}`, 'bad'); setStatus('Demo connection failed', err.message, 'risk'); }
});
$('disconnectDemoBtn').addEventListener('click', disconnectDemo);
$('emergencyBtn').addEventListener('click', () => emergencyStop());
$('autoExecute').addEventListener('change', e => {
  state.autoExecute = e.target.checked;
  log(`Auto-execution ${e.target.checked ? 'ENABLED' : 'disabled'} for DEMO.`, e.target.checked ? 'warn' : '');
  evaluatePair();
});
$('clearLogBtn').addEventListener('click', () => $('log').replaceChildren());

['symbol','barrier','duration','durationUnit','targetPayout','minEdge','stableChecks'].forEach(id => {
  $(id).addEventListener('change', () => {
    if (state.scanning) {
      log('Settings changed. Restart scanner to apply new proposal subscriptions.', 'warn');
      stopScanner();
    }
  });
});
...
log('Sani Arb loaded. Public scanning requires no Deriv token.');
